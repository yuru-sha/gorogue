...
output=$(GOLANGCI_LINT="$fake_lint" make -C "$repo_dir" -s SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" setup-dev 2>&1)
test -z "$output"

write_version 2.13.2
if output=$(GOLANGCI_LINT="$fake_lint" make -C "$repo_dir" -s SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" setup-dev 2>&1); then
