...
GOPATH="$sandbox/gopath" GOMODCACHE="$real_modcache" PATH="$PATH" make -C "$repo_dir" -s GOLANGCI_LINT="$fake_lint" SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" BUILD_DIR="$sandbox/bin" LOG_DIR="$sandbox/logs" STATICCHECK="$fake_staticcheck" GOIMPORTS="$fake_goimports" setup-dev
test -f "$setup_marker" && test -f "$setup_dev_marker"

write_staticcheck_version 2024.1.1
if output=$(GOPATH="$sandbox/gopath" GOMODCACHE="$real_modcache" PATH="$PATH" make -C "$repo_dir" -s GOLANGCI_LINT="$fake_lint" SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" BUILD_DIR="$sandbox/bin" LOG_DIR="$sandbox/logs" STATICCHECK="$fake_staticcheck" GOIMPORTS="$fake_goimports" setup-dev 2>&1); then
...
write_version 2.8.0
GOPATH="$sandbox/gopath" GOMODCACHE="$real_modcache" PATH="$PATH" make -C "$repo_dir" -s GOLANGCI_LINT="$fake_lint" SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" BUILD_DIR="$sandbox/bin" LOG_DIR="$sandbox/logs" STATICCHECK="$fake_staticcheck" GOIMPORTS="$fake_goimports" setup-dev
GOPATH="$sandbox/gopath" GOMODCACHE="$real_modcache" PATH="$PATH" make -C "$repo_dir" -s GOLANGCI_LINT="$fake_lint" SETUP_MARKER="$setup_marker" SETUP_DEV_MARKER="$setup_dev_marker" BUILD_DIR="$sandbox/bin" LOG_DIR="$sandbox/logs" STATICCHECK="$fake_staticcheck" GOIMPORTS="$fake_goimports" ci-checks >/dev/null
