...
grep -Eq 'timeout-minutes: 10' "$workflow"
grep -Eq 'go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2\.13\.2' "$workflow"
! grep -Eq 'golangci/golangci-lint-action@' "$workflow"
grep -Eq 'honnef\.co/go/tools/cmd/staticcheck@2026\.2\.1' "$workflow"
