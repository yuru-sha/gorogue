...
- **セーブファイル削除**: 死亡時の自動削除
- **復活不可**: 真の一回限りのゲーム体験
- **ゲームオーバー遷移**: ターン消費型の行動で死亡した場合もスコアと死因を表示する
- **新規ゲーム開始**: ゲームオーバー後のRestartまたはメニューのNew Gameでは死亡したゲームを再開せず、新しいゲームを開始する
...
#### 機能
- **視界計算**: プレイヤー位置から壁で遮られる範囲を計算
- **探索記憶**: 視界外では探索済み地形だけを表示し、エンティティは表示しない

#### 実装場所
- `internal/game/dungeon/visibility.go` - 視界・探索済み状態の更新
