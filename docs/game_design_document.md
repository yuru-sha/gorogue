...
具体的な要求、対象範囲、除外事項、受け入れ条件は `SPEC.md` を正とします。本書に残る旧版のビジョンやメカニクスは歴史的資料であり、現行実装の保証ではありません。実装状況は `docs/architecture.md` とコード・テストを確認してください。
...
**視界システム（FOV）**
- **壁で遮られる視界**: `Level.UpdateVisibility` がプレイヤー位置から現在の可視範囲を計算
- **探索記録**: `Visible` と `Explored` を分離し、既探索地形を表示
- **エンティティ表示**: アイテムとモンスターは現在可視の範囲だけに表示
